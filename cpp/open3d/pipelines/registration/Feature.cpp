...
CorrespondenceSet CorrespondencesFromFeatures(const Feature &source_features,
    ...
    const int __attribute__((unused)) kOuterThreads = std::min(kMaxThreads, num_searches);
    const int __attribute__((unused)) kInnerThreads = std::max(kMaxThreads / num_searches, 1);
